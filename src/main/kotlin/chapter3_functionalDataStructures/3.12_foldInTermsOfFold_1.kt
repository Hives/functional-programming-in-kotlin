...
fun <A, B> foldRightL(xs: List<A>, acc: B, f: (A, B) -> B): B {
    val reversedXs = List.foldLeft(xs, List.empty<A>()) { z, a -> ConsL(a, z) }
...
fun main() {
    ...
    println(foldRightL(a, List.empty<Int>()) { a, b -> ConsL(a, b) })
    println(List.foldLeft(a, List.empty<Int>()) { a, b -> ConsL(b, a) })
    println(foldLeftR(a, List.empty<Int>()) { a, b -> ConsL(b, a) })
}
